...
@get_result_url
def get_url(ip: list[str], date: int) -> list[str]:
    """
    获取mtbf运行结果的url
    """
    return ip
...
class BaseInfoCreate(object):
    ...
    """
    def __init__(self, ip: list[str], date: int, file_name: str):
        self.url = get_url(ip, date)
...
class GetBugInfo(BaseInfoCreate):
    ...
    """
    def __init__(self, ip: list[str], date: int, file_name: str):
        super().__init__(ip, date, file_name)
...
if __name__ == '__main__':
    args = arg.parse_args()
    gb = GetBugInfo(args.ip_list, args.date, args.name)
    gb.main()
